...
def getTweet():
	...
	if(query and len(query)>0 and query[0] and len(re.sub(r"http\S+", "", query[0].text))>0):
...
def main():
	...
	while True:
		...
		check_tweet = getTweet()
		sleep((280+20)*12*0.04 + 1) # 40 ms to refresh column * ~10 columns for each character * 280+23 characters + plus 1s for tx delay
